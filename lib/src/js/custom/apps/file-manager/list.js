...
var KTFileManagerList = (function () {
        ...
        Swal.fire({
          ...
          if (result.value) {
            Swal.fire({
              ...
              // Remove current row
              const folderId = e.target.closest("tr").getAttribute('data-folder-id');
  ...
  const initToggleToolbar = () => {
    ...
    checkboxes.forEach((c) => {
      ...
      c.addEventListener("click", function () {
        const testObject = {
          item_id: c.value,
          item_type: c.getAttribute("data-type"),
        }

        deletedFolders.push(testObject);
        setTimeout(function () {
          toggleToolbars();
        }, 50);
      });
    ...
    deleteSelected.addEventListener("click", function () {
      ...
      Swal.fire({
        ...
        if (result.value) {
          // Construct the URL with the deletedFolders data
          const url = '/bulk_delete?';
          ...
          Swal.fire({
            text: "Selected files or folders were not deleted.",
            icon: "error",
            buttonsStyling: false,
            confirmButtonText: "Ok, got it!",
            customClass: {
  ...
  function getSlugFromURL() {
    ...
    return urlParams.get("slug"); // Extract the last part (the slug)
  }
        ...
        renameValidator.validate().then(function (status) {
          ...
          if (status == "Valid") {
            ...
            Swal.fire({
              ...
              $.ajax({
                url: `/folders/${folderId}/rename?name=${newFolderName}`,
                type: 'GET',
                processData: false, // Prevent jQuery from processing the data
                contentType: false, // Prevent jQuery from setting the content type
                success: function (data) {
  ...
  function getSelectedFolderId() {
    const selectedRadio = document.querySelector(
      'input[name="move_to_folder"]:checked',
    );
